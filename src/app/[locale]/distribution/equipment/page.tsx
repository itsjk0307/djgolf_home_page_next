...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Distribution.equipment" });
  return { title: t("title") };
}

const BOARD_ID = "distribution3";
const PER_PAGE = 15;
...
export default async function EquipmentPage({
  ...
  const sw = sp.sw ?? "";
  const t = await getTranslations("Distribution.equipment");
  ...
  return (
    ...
    >
      <BoardList boardId={BOARD_ID} posts={posts} total={total} page={page} basePath="/distribution/equipment" searchWord={sw} />
    </SubPageLayout>
  );
}
