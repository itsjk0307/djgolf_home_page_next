...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Distribution.other" });
  return { title: t("title") };
}

const BOARD_ID = "distribution4";
const PER_PAGE = 15;
...
export default async function OtherPage({
  ...
  const sw = sp.sw ?? "";
  const t = await getTranslations("Distribution.other");
  ...
  return (
    ...
    >
      <BoardList boardId={BOARD_ID} posts={posts} total={total} page={page} basePath="/distribution/other" searchWord={sw} />
    </SubPageLayout>
  );
}
