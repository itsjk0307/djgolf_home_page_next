...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Distribution.fertilizer" });
  return { title: t("title") };
}

const BOARD_ID = "distribution1";
const PER_PAGE = 15;
...
export default async function FertilizerPage({
  ...
  const sw = sp.sw ?? "";
  const t = await getTranslations("Distribution.fertilizer");
  ...
  return (
    ...
    >
      <BoardList boardId={BOARD_ID} posts={posts} total={total} page={page} basePath="/distribution/fertilizer" searchWord={sw} />
    </SubPageLayout>
  );
}
