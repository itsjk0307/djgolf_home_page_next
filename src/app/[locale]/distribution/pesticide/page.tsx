...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Distribution.pesticide" });
  return { title: t("title") };
}

const BOARD_ID = "distribution2";
const PER_PAGE = 15;
...
export default async function PesticidePage({
  ...
  const sw = sp.sw ?? "";
  const t = await getTranslations("Distribution.pesticide");
  ...
  return (
    ...
    >
      <BoardList boardId={BOARD_ID} posts={posts} total={total} page={page} basePath="/distribution/pesticide" searchWord={sw} />
    </SubPageLayout>
  );
}
