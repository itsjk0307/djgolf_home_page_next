...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Recruit.jobs" });
...
export default async function JobsPage({
  ...
  const sw = sp.sw ?? "";
  const t = await getTranslations("Recruit.jobs");
  ...
  return (
    ...
    >
      <BoardList boardId={BOARD_ID} posts={posts} total={total} page={page} basePath="/recruit/jobs" searchWord={sw} />
    </SubPageLayout>
  );
}
