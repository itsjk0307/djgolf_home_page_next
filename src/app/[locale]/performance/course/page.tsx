...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "Performance.course" });
  return { title: t("title") };
}

const BOARD_ID = "result1";
const PER_PAGE = 15;
...
export default async function CoursePerformancePage({
  ...
  const sw = sp.sw ?? "";
  const t = await getTranslations("Performance.course");
  ...
  return (
    ...
    >
      <BoardList boardId={BOARD_ID} posts={posts} total={total} page={page} basePath="/performance/course" searchWord={sw} />
    </SubPageLayout>
  );
}
